...
# Get logs from sidecar containers
timeout $LOG_TIMEOUT kubectl logs -l app=sidecar -f --tail 20 -n $NAMESPACE | while read line; do
    echo "LOG: $line"
    if echo "$line" | grep -q "Span captured"; then
        echo -e "${GREEN}✅ SUCCESS: Found 'Span captured' message!${NC}"
        echo "$line"
