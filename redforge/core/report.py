...
class ReportGenerator:
    ...
    def generate_json(self, scan_result: ScanResult) -> Path:
        ...
        filename = f"redforge_scan_{timestamp}.json"
    ...
    def generate_html(self, scan_result: ScanResult) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"redforge_scan_{timestamp}.html"
        output_path = self.output_dir / filename
    ...
    def generate_pdf(self, scan_result: ScanResult) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"redforge_scan_{timestamp}.pdf"
        output_path = self.output_dir / filename
        ...
        story.append(Paragraph("🔥 RedForge Security Scan Report", title_style))
        ...
        # Footer
        story.append(Spacer(1, 0.3*inch))
        footer_text = f"Generated by RedForge CLI v0.2.0 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    def _generate_text_fallback_pdf(self, scan_result: ScanResult, output_path: Path) -> Path:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("REDFORGE SECURITY SCAN REPORT\n")
            f.write("=" * 50 + "\n\n")
...
<head>
    ...
    <title>RedForge Security Scan Report</title>
    <style>
...
<body>
    ...
    <div class="container">
        ...
        <div class="footer">
            <p>Generated by RedForge CLI v0.2.0 | Sprint S-2 MVP</p>
            <p>For support: <a href="mailto:dev@redforge.com">dev@redforge.com</a></p>
        </div>
    </div>
</body>
</html>
