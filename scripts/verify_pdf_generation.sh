...
check_prerequisites() {
    ...
    if command -v poetry &> /dev/null; then
        track_test "Poetry availability" "PASS" "Poetry found at $(command -v poetry)"
    else
        track_test "Poetry availability" "FAIL" "Poetry not found"
        return 1
    fi
    ...
    if poetry run python -c "import reportlab" &> /dev/null; then
        track_test "ReportLab availability" "PASS" "ReportLab installed for PDF generation"
    else
        echo -e "${YELLOW}⚠️  Installing ReportLab for PDF generation...${NC}"
        if poetry add reportlab &> /dev/null; then
            track_test "ReportLab installation" "PASS" "ReportLab installed successfully"
        else
            track_test "ReportLab installation" "FAIL" "Failed to install ReportLab"
            return 1
        fi
    fi
...
verify_github_workflow() {
    ...
    if [ -f "$workflow_file" ]; then
        ...
        # Check file size considerations
...
test_pdf_generation() {
    ...
    local test_cases=(
        ...
        "claude-3-sonnet:audit:Audit report for Claude-3-Sonnet"
    )
...
verify_pdf_content() {
    ...
    # Check PDF metadata using pdfinfo if available
    if command -v pdfinfo &> /dev/null; then
        local page_count=$(pdfinfo "$pdf_file" 2>/dev/null | grep "Pages:" | awk '{print $2}')
        if [ -n "$page_count" ] && [ "$page_count" -gt 0 ]; then
            track_test "PDF content verification ($test_name)" "PASS" "PDF has $page_count pages"
        else
            track_test "PDF content verification ($test_name)" "FAIL" "PDF appears to be empty or corrupted"
        fi
    else
        track_test "PDF content verification ($test_name)" "SKIP" "pdfinfo not available for content verification"
    fi
